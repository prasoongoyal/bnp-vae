    ...
    try:
      ...
      while True:
        ...
        if iteration%10 == 1:
          ...
          with open('embedding_'+str(iteration)+'.txt', 'w') as f:
            for i in range(np.size(embedding, 0)):
              for j in range(np.size(embedding, 1)):
                f.write(str(np.round(embedding[i][j], decimals=2)) + '\t')
              f.write('\n')
